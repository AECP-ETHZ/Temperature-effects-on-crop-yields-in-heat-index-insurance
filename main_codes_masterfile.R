...
# Approximation of CE and RP
# Shows which moment has the stronger reduction

# Approximation of risk premium based on second (variance) and third (skewness) moment
# Both variance and negative skewness increase the risk premium
'Hier dann auch noch für alpha = 1 machen'
RP_approx_function <- function(alpha,revenue){
  1/2 * (alpha / mean(as.numeric(revenue), na.rm=T) * var(as.numeric(revenue), na.rm=T) + 1/6 * (-(alpha^2 + alpha)/ (mean(as.numeric(revenue), na.rm=T)^2)) * skewness(as.numeric(revenue), na.rm=T))
}
...
summary_EU_wheat_uninsured_approximated <- as.data.frame(matrix(NA, nrow=nrow(farm_yields[[1]]),ncol=3))
colnames(summary_EU_wheat_uninsured_approximated) <- c("Expected Revenue", "Risk premium", "CE")

for (i in 1:nrow(farm_yields[[1]])){
  
  # Expected Revenue
  summary_EU_wheat_uninsured_approximated [i,1] <- mean(as.numeric(detr_wheat_yields[i,]), na.rm=T)
  
  # Risk premium
  summary_EU_wheat_uninsured_approximated [i,2] <- RP_approx_function(alpha = alpha, revenue = detr_wheat_yields[i,])
   
  # Certainty equivalent
  summary_EU_wheat_uninsured_approximated [i,3] <- summary_EU_wheat_uninsured_approximated [i,1] - summary_EU_wheat_uninsured_approximated [i,2] 
}
...
summary_EU_rapeseed_uninsured_approximated <- as.data.frame(matrix(NA, nrow=nrow(farm_yields[[1]]),ncol=3))
colnames(summary_EU_rapeseed_uninsured_approximated) <- c("Expected Revenue", "Risk premium", "CE")

for (i in 1:nrow(farm_yields[[1]])){
  
  # Expected Revenue
  summary_EU_rapeseed_uninsured_approximated [i,1] <- mean(as.numeric(detr_rapeseed_yields[i,]), na.rm=T)
  
  # Risk premium
  summary_EU_rapeseed_uninsured_approximated [i,2] <- RP_approx_function(alpha = alpha, revenue = detr_rapeseed_yields[i,])
  
  # Certainty equivalent
  summary_EU_rapeseed_uninsured_approximated [i,3] <- summary_EU_rapeseed_uninsured_approximated [i,1] - summary_EU_rapeseed_uninsured_approximated [i,2] 
}
...
summary_EU_wheat_insured_kn3_approximated <- list() # [[strike]] [m,i,c(EU,E,CE,RP)]

for (strike in 1:length(strike_temperature)){
  temp_array <- array(dim=c(ncol(list_knot_locations_kn3_wheat[[1]]), nrow(farm_yields[[1]]), 3))
  dimnames(temp_array) [[3]] <- c("Expected Revenue", "RP", "CE")
  
  for (m in 1:ncol(list_knot_locations_kn3_wheat[[1]])){
    for (i in 1:nrow(farm_yields[[1]])){
      
      # Expected Revenue
      temp_array  [m,i,1] <- mean(as.numeric(list_farm_wheat_wealth_modelskn3_strike[[strike]][m,i,]), na.rm=T)
      
      # Risk premium
      temp_array  [m,i,2] <- RP_approx_function(alpha=alpha, revenue=list_farm_wheat_wealth_modelkn3_strike_utility[[strike]][m,i,])
      
      # Certainty equivalent
      temp_array  [m,i,3] <- temp_array  [m,i,1] -  temp_array  [m,i,2]
    }
  }
  summary_EU_wheat_insured_kn3_approximated [[strike]] <- temp_array
  rm(temp_array)
}
...
# with RP approximation formula
summary_EU_wheat_insured_kn5_approximated <- list() # [[strike]] [m,i,c(EU,E,CE,RP)]

for (strike in 1:length(strike_temperature)){
  temp_array <- array(dim=c(length(knot_locations_kn5_wheat), nrow(farm_yields[[1]]), 3))
  dimnames(temp_array) [[3]] <- c("Expected Revenue", "RP", "CE")
  ...
  for (m in 1:ncol(knot_locations_kn5_wheat)){
    for (i in 1:nrow(farm_yields[[1]])){
      ...
      # Expected Revenue
      temp_array  [m,i,1] <- mean(as.numeric(list_farm_wheat_wealth_modelskn5_strike[[strike]][m,i,]), na.rm=T)
      
      # Risk premium
      temp_array  [m,i,2] <- RP_approx_function(alpha=alpha, revenue = list_farm_wheat_wealth_modelskn5_strike[[strike]][m,i,])
      
      # Certainty equivalent
      temp_array  [m,i,3] <- temp_array  [m,i,1] -  temp_array  [m,i,2]
    }
  }
  summary_EU_wheat_insured_kn5_approximated [[strike]] <- temp_array
  rm(temp_array)
}
